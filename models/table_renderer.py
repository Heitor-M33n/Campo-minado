...
class Table_renderer():
    ...
    def render_field(self, render_data: dict) -> Table:
        return self.__generate_table(render_data, render_data['field'])
    
    def render_visible_field(self, render_data: dict) -> Table:
        return self.__generate_table(render_data, render_data['visible field'])

    @classmethod
    def __generate_table(cls, render_data: dict, field: list) -> Table:
        ...
        for row in field:
            ...
            for item in row:
                new_row.append(cls.REPLACES.get(item, item))

            table_field.append(new_row)                

        for i in range(render_data['width']):
            y = Text(str(render_data['width'] - i), '')
            row = list(Text(str(item), f'{cls.COLORS.get(item, '')}') for item in table_field[i])
